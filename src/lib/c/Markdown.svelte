...
<script lang="ts">
     ...
     * div元素
     */
    let self_div: HTMLDivElement

    /**
     * 是否固定左边的index
     */
    let fixed_left_index = writable(false)
     ...
     */
    function on_scroll() {
        fixed_left_index.set(scroll_y >= self_div.offsetTop)
    }
</script>

<svelte:window bind:scrollY={scroll_y} on:scroll={on_scroll} />

<div class="markdown" bind:this={self_div}>
    <div class="markdown-inner">
        <div class={$fixed_left_index ? 'markdown-fixed' : 'markdown-index'}>
            {@html $html_content_toc}
        </div>
